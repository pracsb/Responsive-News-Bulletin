 ...
 class Business extends Component {
    ...
    async get_api_response(checkedState){
        ...
        if(checkedState==true)
        {
            ...
            await fetch('https://hw8webtech.wl.r.appspot.com/guardian_business')
            .then(res=>res.json())
            .then(res => {
                if(this._isMounted)
                {
        ...
        if(checkedState==false)
        {
            ...
            this.setState({apiResponse: []})
            await fetch('https://hw8webtech.wl.r.appspot.com/NY_business')
            .then(res=>res.json())
            .then(res => {
                if(this._isMounted)
                {
    ...
    render() {
        ...
        :
            (this.state.apiResponse.map((news) => 
            {
            return  <Link style={{textDecoration:"none"}} key={news.url} to={"/article?"+ news.url}><Card onChildClick={this.handleChildClick.bind(this)} newsFromApp={news} key={news.url} image={this.get_image_World(news.multimedia)} title={news.title} description={news.abstract} date={Moment(news.published_date).format('YYYY-MM-DD')} tag={news.section} shareURL={news.url}/></Link>;
            }))
